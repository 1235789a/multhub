  ...
  return (
    <Layout env={env} title="Discover handmade products" path="/discovery" description="A curated discovery space for handmade products with clear stories, materials, customization details, and gift occasions.">
      <section class="page-hero section-shell">
        ...
        <h1>Meet the product <em>and the maker.</em></h1>
        <p>Handmade products with the materials, process, options, and ordering details gathered in one place.</p>
      </section>
      ...
      <section class="section section-shell discovery-grid">
        {products.length === 0 ? (
          <div class="empty-state"><span>✦</span><h2>We’re preparing the first products.</h2><p>Send yours for a review if you would like us to consider it for this shelf.</p><a class="button" href="/#review">Send a product for review</a></div>
...
function ProductCard({ product }: { product: Product }) {
  return (
    <article class="discovery-card">
      ...
      <div class="discovery-card-copy"><p class="card-kicker">{product.category}</p><h2><a href={`/products/${product.slug}`}>{product.name}</a></h2><p>{product.short_story}</p><div class="maker-line"><span>{product.seller_name ?? "Independent maker"}</span><a href={`/products/${product.slug}`}>View product →</a></div></div>
    </article>
  );
}
  ...
  return (
    <Layout env={env} title={product.name} path={`/products/${product.slug}`} description={product.short_story} image={product.image_url ?? "/social-card.svg"} structuredData={[productSchema, ...(faqSchema ? [faqSchema] : [])]}>
      <article class="passport-page section-shell">
        ...
        <aside class="passport-disclosure"><strong>About these details</strong><p>The maker supplied the product information on this page. Confirm custom options and lead times directly before ordering.</p></aside>
      </article>
    </Layout>
  );
}
...
export function AiReadyPage({ env }: { env: Bindings }) {
  const faq = [
    ["What makes a product page search-ready?", "A clear product name, specific details, useful headings, buyer questions, and consistent information about materials, size, options, care, and ordering."],
    ["Is this the same as SEO?", "It includes the basics of good product-page SEO, but the work starts with the information itself. A well-structured page cannot make up for vague or missing product details."],
    ["How do AI assistants use a product page?", "They may read the public text, metadata, links, and structured data available on the page. Clear source information makes it easier to describe the product accurately."],
  ];
  return (
    <Layout env={env} title="Search-ready product pages" path="/ai-ready" description="How clear product details, buyer questions, metadata, and structured data help search engines and AI assistants understand handmade products." structuredData={{ "@context": "https://schema.org", "@type": "FAQPage", mainEntity: faq.map(([q, a]) => ({ "@type": "Question", name: q, acceptedAnswer: { "@type": "Answer", text: a } })) }}>
      <section class="page-hero section-shell"><p class="eyebrow">Search and AI discovery</p><h1>Help search tools understand <em>your product.</em></h1><p>A useful source page answers the same questions a buyer would ask: what it is, how it is made, what can change, who it suits, and how to order.</p></section>
      <section class="section section-shell explainer-grid"><article><span>01</span><h2>Name the product clearly</h2><p>Use the words a buyer would recognize, then state the materials, size, method, and options without making them hunt through captions.</p></article><article><span>02</span><h2>Answer buying questions</h2><p>Turn repeated messages about care, customization, timing, and delivery into useful page content.</p></article><article><span>03</span><h2>Structure the page</h2><p>Headings, metadata, internal links, and relevant Schema.org fields help tools identify each piece of information.</p></article><article><span>04</span><h2>Keep the facts consistent</h2><p>The product name, options, and claims should agree across your shop, social posts, FAQs, and maker profile.</p></article></section>
      <section class="section boundary-section"><div class="section-shell"><div class="section-heading narrow"><p class="eyebrow">What this work is for</p><h2>A clearer source page for <em>every channel.</em></h2><p>When a buyer, search engine, or AI assistant reaches the page, the product name, materials, options, process, and ordering details should agree. That makes the item easier to understand and easier to describe accurately.</p></div></div></section>
      <section class="section section-shell faq-section"><div class="faq-list">{faq.map(([q, a]) => <details open><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</div><div class="center-cta"><a class="button" href="/#review">Start with a free product review →</a></div></section>
    </Layout>
  );
}
