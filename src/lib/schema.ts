...
/**
 * Keep a newly-bound or accidentally-unmigrated D1 database usable.
 * Wrangler migrations remain the normal deployment path; this is a safe,
 * idempotent fallback so a deployment cannot accept traffic with no tables.
 */
export function ensureDatabase(db: D1Database): Promise<void> {
  ...
  const initialization = initializeDatabase(db).catch((error) => {
    readyDatabases.delete(key);
    throw error;
  });
  readyDatabases.set(key, initialization);
...
async function initializeDatabase(db: D1Database): Promise<void> {
  ...
  // Older databases may have migration 0001 without this later field.
  const columns = await db.prepare("PRAGMA table_info(leads)").all<{ name: string }>();
  if (!columns.results.some((column) => column.name === "is_founding_client")) {
    await db.prepare("ALTER TABLE leads ADD COLUMN is_founding_client INTEGER NOT NULL DEFAULT 0").run();
  }
}

export async function databaseHealth(db: D1Database): Promise<boolean> {
  await ensureDatabase(db);
  const result = await db.prepare("SELECT 1 AS ok FROM leads LIMIT 1").first<{ ok: number }>();
  // An empty leads table is healthy; D1 returns null because LIMIT 1 has no row.
  if (result === null) {
    const table = await db.prepare("SELECT 1 AS ok FROM sqlite_master WHERE type = 'table' AND name = 'leads'")
      .first<{ ok: number }>();
    return table?.ok === 1;
  }
  return result.ok === 1;
